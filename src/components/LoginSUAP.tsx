...
interface LoginSUAPProps {
  onLogin: (matricula: string, senha: string) => void;
  onBack: () => void;
  onSwitchToEmail?: () => void;
}

export function LoginSUAP({ onLogin, onBack, onSwitchToEmail }: LoginSUAPProps) {
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-gray-600 text-center text-sm">
              Acesso institucional SUAP
            </p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="matricula" className="block text-gray-700 mb-2 text-sm">
                Código SUAP
              </label>
              <input
                id="matricula"
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white py-4 rounded-xl transition-all shadow-md hover:shadow-lg text-base mt-6"
            >
              Entrar
            </button>
          </form>
